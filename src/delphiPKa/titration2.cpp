...
void CTitration2 :: run()
{
    ...
    for (fPH=pH_initial; fPH<=pH_end ; fPH+=pH_step) {
        ...
        for (i=MPI_START; i<MPI_END; i++) {  // MPI version loop
            ...
            for (j=0; j<vecState.size(); j++) {
                ...
                ergrxn = 0.0; ergpolar = 0.0; erg0 = 0.0;
                ...
                for (k=0; k<vecPair[i].size(); k++) {
                    ...
                    pos = find(vecCluster1[i].begin(), vecCluster1[i].end(), vecPair[i][k].res1) - vecCluster1[i].begin();
                    ...
                    pos = find(vecCluster1[i].begin(), vecCluster1[i].end(), vecPair[i][k].res2) - vecCluster1[i].begin();
        ...
        MPI::COMM_WORLD.Gatherv( &vecProbMPI_partial[0], count, MPI::FLOAT, &vecProbMPI[0], rcount, displs, MPI::FLOAT, 0 );
    ...
    if(id==0){
        ...
        for(i=0;i<vec2dProb.size();i++) {
            ...
            probOutput << setfill('0') << setw(4) << right << newPDB[vecIonRes[i][0]].res_num; probOutput << setfill(' ');
				...
				if(newPDB[vecIonRes[j][0]].res_name == "A" ||
				   ...
				   newPDB[vecIonRes[j][0]].res_name == "DC") { total_netCrg += vec2dProb[j][i] * 1.0 + (-1.0); }
					...
					if(newPDB[vecIonRes[j][0]].res_name == "LYS" ||
				   	   ...
				   	   newPDB[vecIonRes[j][0]].res_name == "HIS")	sign = 1;
					else if(newPDB[vecIonRes[j][0]].res_name == "ASP" ||
					    	newPDB[vecIonRes[j][0]].res_name == "GLU")	sign = -1;
