...
void CPlaceHydrogen :: run()
{
    ...
    for(it_pdb=PDB.begin();it_pdb!=PDB.end();++it_pdb)
    {
        ...
        if(it_pdb->atom_name=="N" && it_pdb->res_num!=1){  // when atom is N, it requires to search the connected atom in previous residue;
            ...
            if(value_found.orbital=="sp3" && found_bond_num==1) {
                ...
                if(map.find(next_key)->second.bondatom3 == it_pdb->atom_name) {
                    pdb_search_key = it_pdb->chain_id+" "+to_string(it_pdb->res_num)+" "+map.find(next_key)->second.bondatom2;
                    Vector2 = mapPDB.find(pdb_search_key)->second.coord;
                }
                else if (map.find(next_key)->second.bondatom2 == it_pdb->atom_name) {
                    pdb_search_key = it_pdb->chain_id+" "+to_string(it_pdb->res_num)+" "+map.find(next_key)->second.bondatom1;
                    Vector2 = mapPDB.find(pdb_search_key)->second.coord;
                }
                ...
                else if (num_of_H_to_add==1) {   // It's for the case SER OG/THR OG1/TYR OH which is sp3 orbital and one H to predict
                    atom_to_insert.res_name  = it_pdb->res_name;
                    atom_to_insert.chain_id  = it_pdb->chain_id ;
                    atom_to_insert.res_num   = it_pdb->res_num;
