...
void CTitration2 :: linearReg()
{
    ...
    pkaOuptut << " ResName           pKa    polar(charged) polar(neutral) de-solvation(charged) de-solvation(neutral)" << endl;
    ...
    for (i=0; i<vec2dProb.size(); i++) {
        ...
        if(vecX.size() == 0) {
            ...
            for(j=0;j<n;j++) { x[j] = vecX[j]; y[j] = vecY[j]; }
        ...
        pkaOuptut << setfill('0') << setw(4) << right << newPDB[vecIonRes[i][0]].res_num; pkaOuptut << setfill(' ');
...
void CTitration2 :: genCluster1()
{
	...
	vector<string> vecIonRes1dChID; 
    vector<int> vectmp;
    
    for(i=0; i<vecIonRes.size(); i++) {
        vecIonRes1d.push_back(newPDB[vecIonRes[i][0]].res_num);
		vecIonRes1dChID.push_back(newPDB[vecIonRes[i][0]].chain_id);
    }
    ...
    for(i=0; i<vecCluster.size(); i++) {
        for(j=0; j<vecCluster[i].size(); j++) {
            for(k=0; k<vecIonRes1d.size(); k++) {
                if(vecCluster[i][j] == vecIonRes1d[k] && vecClusterChID[i][j] == vecIonRes1dChID[k])
                    vectmp.push_back(k);
            }
        }
...
float CTitration2 :: genEnergy0(int& n, float& fPH)
{
    ...
    if("ARG" == resnam || "HIS" == resnam || "LYS" == resnam || "A" == resnam || "C" == resnam || "DA" == resnam || "DC" == resnam) {
        iGamma = 1.0;
